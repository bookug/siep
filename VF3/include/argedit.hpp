...
*/
template<typename Node, typename Edge> void ARGEdit<Node, Edge>::InsertEdge(node_id id1, node_id id2, Edge& attr){
  eNode *e=new eNode;  
  if (e==NULL)  error("Out of memory"); 
  e->from=id1;  e->to=id2;  e->attr=attr;  e->pos=-1;
  nNode *pn=nodes;
  if (lastNode!=NULL && lastNode->id<=id1)  pn=lastNode;
  while (pn!=NULL && pn->id<id1)  pn=pn->next;
  if (pn==NULL || pn->id!=id1)   error("Bad param 1 in ARGEdit::InsertEdge: %d", (int)id1);
  ...
  if (p!=NULL && p->to==id2)  error("Bad param 2 in ARGEdit::InsertEdge: %d", (int)id2);  
  if (p0==NULL){
    e->next=pn->edges;
    pn->edges=e;
  }
