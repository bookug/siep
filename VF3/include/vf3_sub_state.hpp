 ...
 * See vf2_state.cc for more details.
 ---------------------------------------------------------*/
template <typename Node1, typename Node2, typename Edge1, typename Edge2, typename NodeComparisonFunctor = EqualityComparator<Node1, Node2>, typename EdgeComparisonFunctor = EqualityComparator<Edge1, Edge2> >
class VF3SubState {
private:
...
public:
	...
	VF3SubState(ARGraph<Node1, Edge1> *g1, ARGraph<Node2, Edge2> *g2, int* class_1, int* class_2, int nclass, node_id* order = NULL);
 ...
 * Methods of the class VF3SubState
 ---------------------------------------------------------*/
template <typename Node1, typename Node2, typename Edge1, typename Edge2, typename NodeComparisonFunctor, typename EdgeComparisonFunctor>
...
VF3SubState<Node1,Node2,Edge1,Edge2,NodeComparisonFunctor,EdgeComparisonFunctor>::VF3SubState(ARGraph<Node1, Edge1> *ag1, ARGraph<Node2, Edge2> *ag2, int *class_1, int *class_2, int nclass, node_id *order) {
	...
	memset(t1both_len, 0, (n1+1)*sizeof(int));
	t1in_len = new int[n1+1];
	memset(t1in_len, 0, (n1+1)*sizeof(int));
	t1out_len = new int[n1+1];
	memset(t1out_len, 0, (n1+1)*sizeof(int));
	
	termin1 = new int[n1];//(int*) calloc(n1, sizeof(int));
	memset(termin1, 0, n1*sizeof(int));
	termout1 = new int[n1];//(int*) calloc(n1, sizeof(int));
	memset(termout1, 0, n1*sizeof(int));
	new1 = new int[n1];//(int*) calloc(n1, sizeof(int));
	memset(new1, 0, n1*sizeof(int));
	
	t1both_len_c = new int*[n1+1];//(int**)malloc((n1+1)*sizeof(int*));
	memset(t1both_len_c, 0, (n1+1)*sizeof(int));
	t1in_len_c = new int*[n1+1]; //(int**)malloc((n1+1)*sizeof(int*));
	memset(t1in_len_c, 0, (n1+1)*sizeof(int));
	t1out_len_c = new int*[n1+1]; //(int**)malloc((n1+1)*sizeof(int*));
	memset(t1out_len_c, 0, (n1+1)*sizeof(int));
	
	termin1_c = new int*[n1]; //(int**)malloc(n1*sizeof(int*));
	memset(termin1_c, 0, n1*sizeof(int));
	termout1_c = new int*[n1]; //(int**)malloc(n1*sizeof(int*));
	memset(termout1_c, 0, n1*sizeof(int));
	new1_c = new int*[n1]; //(int**)malloc(n1*sizeof(int*));
	memset(new1_c, 0, n1*sizeof(int));
	
	core_len_c = new int[classes_count+1]; //   (int*) calloc(classes_count+1, sizeof(int));
	memset(core_len_c, 0, (classes_count+1)*sizeof(int));
	...
	t2in_len_c = new int[classes_count+1]; //   (int*) calloc(classes_count+1, sizeof(int));
	memset(t2in_len_c, 0, (classes_count+1)*sizeof(int));
	t2out_len_c =  new int[classes_count+1]; // (int*) calloc(classes_count+1, sizeof(int));
	memset(t2out_len_c, 0, (classes_count+1)*sizeof(int));
	termout2_c = new int[classes_count+1];
	memset(termout2_c, 0, (classes_count+1)*sizeof(int));
	termin2_c = new int[classes_count+1];
	memset(termin2_c, 0, (classes_count+1)*sizeof(int));
	new2_c = new int[classes_count+1];
	memset(new2_c, 0, (classes_count+1)*sizeof(int));
	...
	for(i=0; i<=n1; i++)
	  ...
	  if(i<n1){
		core_1[i]=NULL_NODE;
		  termin1_c[i] =  new int[classes_count+1];// (int*) calloc(classes_count+1, sizeof(int));
		  memset(termin1_c[i], 0, (classes_count+1)*sizeof(int));
		  termout1_c[i] = new int[classes_count+1];// int*) calloc(classes_count+1, sizeof(int));
		  memset(termout1_c[i], 0, (classes_count+1)*sizeof(int));
		  new1_c[i] = new int[classes_count+1];// (int*) calloc(classes_count+1, sizeof(int));
		  memset(new1_c[i], 0, (classes_count+1)*sizeof(int));
	  }
		t1both_len_c[i] = new int[classes_count+1];// (int*) calloc(classes_count+1, sizeof(int));
		memset(t1both_len_c[i], 0, (classes_count+1)*sizeof(int));
		t1in_len_c[i] = new int[classes_count+1];// (int*) calloc(classes_count+1, sizeof(int));
		memset(t1in_len_c[i], 0, (classes_count+1)*sizeof(int));
		t1out_len_c[i] = new int[classes_count+1];// (int*) calloc(classes_count+1, sizeof(int));
		memset(t1out_len_c[i], 0, (classes_count+1)*sizeof(int));
	  }
...
VF3SubState<Node1,Node2,Edge1,Edge2,NodeComparisonFunctor,EdgeComparisonFunctor>::~VF3SubState() {
	...
	if (*share_count == 0){ 
		...
		delete [] in_2;		in_2=NULL;
		delete [] out_2;	out_2=NULL;
		delete [] dir;		dir=NULL;
		delete [] predecessors;	predecessors=NULL;
		delete [] t1both_len;	t1both_len=NULL;
		delete [] t1in_len;	t1in_len=NULL;
		delete [] t1out_len;	t1out_len=NULL;
		delete [] termin1;	termin1=NULL;
		delete [] termout1;	termout1=NULL;
		delete [] new1;		new1=NULL;
		
		for(int i = 0; i <= n1; i++){
			delete [] t1both_len_c[i];	t1both_len_c[i]=NULL;
			delete [] t1in_len_c[i];	t1in_len_c[i]=NULL;
			delete [] t1out_len_c[i];	t1out_len_c[i]=NULL;
			if(i< n1){
				delete [] termin1_c[i];	termin1_c[i]=NULL;
				delete [] termout1_c[i];termout1_c[i]=NULL;
				delete [] new1_c[i];	new1_c[i]=NULL;
			}
		}
		
		delete [] t1both_len_c;	t1both_len_c=NULL;
		delete [] t1in_len_c;	t1in_len_c=NULL;
		delete [] t1out_len_c;	t1out_len_c=NULL;
		delete [] termin1_c;	termin1_c=NULL;
		delete [] termout1_c;	termout1_c=NULL;
		delete [] new1_c;	new1_c=NULL;
		delete [] t2both_len_c;	t2both_len_c=NULL;
		delete [] t2in_len_c;	t2in_len_c=NULL;
		delete [] t2out_len_c;	t2out_len_c=NULL;
		delete [] core_len_c;	core_len_c=NULL;
		delete [] termin2_c;	termin2_c=NULL;
		delete [] termout2_c;	termout2_c=NULL;
		delete [] new2_c;	new2_c=NULL;
...
//NOTICE: we choose the definition of embedding subgraph isomorphism, i.e. monomorphism
	...
	//1. check the vertex labels
	//The nf() is NodeComparisonFunctor (should be set by users)
...
//This part is commented out because they are feasible only on induced subgraph isomorphism
//{
