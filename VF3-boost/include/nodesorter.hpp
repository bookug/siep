...
// class NodeSorter {
//   public:
//     virtual vector<node_id> SortNodes(ARGraph<data_t,Empty>* pattern)=0;
// };
...
// class VF2NodeSorter:NodeSorter {
//   public:
//     vector<node_id> SortNodes(ARGraph<data_t,Empty>* pattern);
// };
...
class VF3NodeSorter {
public:  
	vector<node_id> SortNodes(Graph *query) {
		int nodeCount;
		nodeCount = query->vertex_num;
		...
		for(int i = 0; i < nodeCount; i++) {
			...
			nodes[i].deg = query->vertices[i].neigh.size();
			nodes[i].deg_neigh = 0;
			nodes[i].probability = 1/query->vertex_num;;
		...
		AddNodeToSortedSet(query, top, n, nodes, candidates, nodes_order);
		...
		for (; n < nodeCount - 1; n++) {
			...
			if ((*candidate_it)->used) {
				...
				AddNodeToSortedSet(query, (*candidate_it)->id, n, nodes, candidates, nodes_order);
			}
	...
	void AddNodeToSortedSet(Graph *query, node_id node, node_id level, 
		...
		count = query->vertices[node].neigh.size();
	
		//Updating Inner Nodes not yet inserted
		for (int i = 0; i < count; i++) {
			//Getting Neighborhood
			neigh = query->vertices[node].neigh[i].vid;// pattern->GetInEdge(node,i);
			if (!nodes[neigh].used)
				nodes[neigh].deg_m++;
...
// /****************** VF2 SORTER ********************/
// vector<node_id> VF2SortNodes(ARGraph<data_t,Empty>* pattern) {
//   int nodeCount;
//   node_id i, k, neigh;
