...
template<typename State>
bool match(State &s, int *pn, node_id c1[], node_id c2[],
           match_visitor vis, void *usr_data, long& num_recursive_call)
{
    if (nembeddings > bound) return false;
    num_recursive_call++;
  if (s.IsGoal())
  {
    ++*pn;
  ...
  while (s.NextPair(&n1, &n2, n1, n2)){
	...
	State s1(s);
	s1.AddPair(n1, n2);
    	if (match(s1, pn, c1, c2, vis, usr_data, num_recursive_call));
      	//return true;
    }
  }
  return false;
