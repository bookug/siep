...
Match::Match(Graph* _query_graph_ptr, Graph* _data_graph_ptr, IO* _io_ptr, HyperGraph* _gsh)
{
    ...
    this->data_v_visited = vector<bool>(_data_graph_ptr->vertex_num, false);
}
...
{
	if (numofembeddings > bound) return 0;
    if(pos == qVNum)
    {
double vsize, rss;
...
if (vsize > max_mem)
	...
	if (numofembeddings > bound) return sum;
    }
    M[pos] = hv;
	//for(int i = 0; i < qVNum; ++i) {
		//fprintf(fpR, "(%d, %d) ", i, M[i]);
...
bool Match::quicksi(int _depth) {
    ...
    if(_depth == this->query_v_num) {
        // found a match
        //this->try_res_output();
        int ret = output(this->data_v_visited, this->matched_data_vid_arr, this->query_v_num, this->ofp, gsh, 0, this->seq.entries[0].vid);
        //numofembeddings += ret;
        //double vsize, rss;
		//Util::process_mem_usage(vsize, rss);
		//if (vsize > max_mem)
    ...
    //initialize candidates of this step by utilizing se edges of parent
