...
DataVertex::DataVertex(int _vid,int _vlabel)
{
	this->vid=_vid;
	this->vlabel=_vlabel;
}
...
void DataGraph::add_vertex(int _vid,int _vlb)
{
	this->vertices.push_back(DataVertex(_vid,_vlb));
}

void DataGraph::add_edge(int  _from_vid, int _to_vid)
{
	// no direction
	this->vertices[_from_vid].neighbors.push_back(Neighbor(_to_vid));
	this->vertices[_to_vid].neighbors.push_back(Neighbor(_from_vid));
